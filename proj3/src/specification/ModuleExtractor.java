...
public class ModuleExtractor {
	...
	public ModuleExtractor(OWLOntology ontology, boolean dualConcepts, boolean dualRoles, 
		...
		//Set localityChecker
		setLocalityChecker(dualConcepts, dualRoles, considerEntityAnnotations,
				ignoreAssertions);
	}
	...
	private void setLocalityChecker(boolean dualConcepts, boolean dualRoles, boolean considerEntityAnnotations,
									boolean ignoreAssertions, OWLDataFactory dataFactory) {
		localityChecker = new SyntacticLocalityChecker(dualConcepts, dualRoles, considerEntityAnnotations,
				ignoreAssertions, dataFactory);
	}
	...
	public OWLOntology getLocalityModuleForSignatureGroup(Set<OWLEntity> signature){
		return getLocalityModuleForSignatureGroup(signature,
				"http://krono.ac.uji.es/Links/ontologies/module-" +
						Calendar.getInstance().getTimeInMillis() + "-" + Math.random()  + ".owl"); //new UID()
	}
	...
	public OWLOntology getLocalityModuleForSignatureGroup(Set<OWLEntity> signature, String moduleURIStr,
		...
		//System.out.println("Time extract module axioms (" + moduleAxioms.size() + " axioms) (s): "  +
		// (double)((double)fin-(double)init)/1000.0);
