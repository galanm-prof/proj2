...
public class experimentC_4c{
    ...
    private static void initRequests(){
     ...
     private static void search(Set<String>repositories) throws IOException, OWLOntologyCreationException {
         ...
         FileWriter g = new FileWriter("./src/experimentsPost/experimentC_4c.out");
     ...
     private static boolean experiment() throws IOException, OWLOntologyCreationException {
         boolean r = false;
         Tuple2<Boolean,Set<String>> tuple=experimentC_4c.selectionOfRepositories();
         if (!tuple.getField1()) {
             System.out.println("Before generating repositories: "+tuple.getField2().size()+"/"+nRepositories);
             boolean b=experimentC_4c.generateRepositories();
             System.out.println("Generation of repositories: "+b);
         }
         initRequests();
         tuple = experimentC_4c.selectionOfRepositories();
         r = tuple.getField1();
         if (r)
             experimentC_4c.search(tuple.getField2());
         return r;
     }
     public static void main(String[]args){
         try {
             boolean r=experimentC_4c.experiment();
