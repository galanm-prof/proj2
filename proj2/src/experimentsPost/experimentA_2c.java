...
public class experimentA_2c{
    ...
    private static void initRequests(){
     ...
     private static void search(Set<String>repositories) throws IOException, OWLOntologyCreationException {
         ...
         FileWriter g = new FileWriter("./src/experimentsPost/experimentA_2c.out");
     ...
     private static boolean experiment() throws IOException, OWLOntologyCreationException {
         boolean r = false;
         Tuple2<Boolean,Set<String>> tuple=experimentA_2c.selectionOfRepositories();
         if (!tuple.getField1()) {
             System.out.println("Before generating repositories: "+tuple.getField2().size()+"/"+nRepositories);
             boolean b=experimentA_2c.generateRepositories();
             System.out.println("Generation of repositories: "+b);
         }
         initRequests();
         tuple = experimentA_2c.selectionOfRepositories();
         r = tuple.getField1();
         if (r)
             experimentA_2c.search(tuple.getField2());
         return r;
     }
     public static void main(String[]args){
         try {
             boolean r=experimentA_2c.experiment();
